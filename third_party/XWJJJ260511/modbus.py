...
# logging 用来输出串口连接、寄存器读写、运动状态等调试信息。
import logging
# time 用来做状态等待、运动等待和短暂延时。
import time
# pymodbus 的串口客户端，负责真正的 Modbus RTU 通信。
from pymodbus.client import ModbusSerialClient
# ModbusException 用来捕获 pymodbus 抛出的通信异常。
from pymodbus.exceptions import ModbusException

# 设置默认日志级别为 INFO，所以你会看到“读取保持寄存器成功”等日志。
logging.basicConfig(level=logging.INFO)
# 当前模块的日志对象，日志前缀会显示 XWJJJ260511.modbus。
...
class ModbusRTUClient:
    ...
    def connect(self) -> bool:
        ...
        if self._connected:
            ...
            return True
        try:
            # pymodbus 串口模式依赖 pyserial；这里提前检查，报错更清楚。
            import serial  # noqa: F401
        except ImportError:
            logger.error("缺少串口依赖 pyserial，请先运行: pip install -r requirements.txt")
            self._connected = False
            return False
        # 创建 Modbus RTU 串口客户端。
    ...
    def _write_32bit(self, slave: int, reg_high: int, value: int) -> bool:
        ...
        # 驱动器用两个 16 位寄存器表示一个 32 位数；负数先转补码。
        if value < 0:
            value = value + (1 << 32)
        # 高 16 位写到 reg_high。
    ...
    def _read_32bit(self, slave: int, reg_high: int) -> int | None:
        ...
        # 拼成 32 位无符号整数。
        val = (high << 16) | low
        # 如果最高位为 1，按 32 位有符号数还原为负数。
        if val & (1 << 31):
            val = val - (1 << 32)
        return val

    def __enter__(self):
        # 支持 with ModbusRTUClient(...) as client: 自动连接。
        self.connect()
        return self
    ...
    def enable_motor(self, slave: int) -> bool:
        ...
        # 某些驱动器需要先禁止电压，才能重新按 CiA402 序列使能。
        ...
        # 标准使能序列
        # 1) Shutdown：进入 Ready to switch on。
        ...
        # 3) Enable operation：进入 Operation enabled，可以运动。
    ...
    def switch_mode(self, slave: int, mode_value: int, auto_enable: bool = True) -> bool:
        ...
        # 需要自动使能时，切完模式后进入 Operation enabled。
        if auto_enable:
    ...
    def _restore_enabled_state(self, slave: int, timeout: float = 1.0) -> bool:
        """运动结束后恢复使能状态。"""
        # 写 Enable operation 控制字。
        if not self._write_controlword(slave, self.CMD_ENABLE_OPERATION):
            logger.error("恢复使能状态失败：写入控制字失败")
            return False
        # 在短时间内等待状态字出现 Operation enabled 位。
        start = time.time()
        while time.time() - start < timeout:
            status = self._read_statusword(slave)
            if status is not None and (status & self.STAT_OPERATION_ENABLED):
    ...
    def move_absolute_pp(self, slave: int, target_pos: int,
        ...
        # 0x10 是“新设定点”位，告诉驱动器开始执行刚写入的目标位置。
        ...
        while time.time() - start_time < timeout:
            ...
            if current_pos is None:
                time.sleep(0.05)
                continue
            # 允许 50 脉冲误差，避免因为微小跟随误差永远等不到完全相等。
        ...
        # 计算位置误差
        # final_pos 是反馈当前位置，cmd_pos 是驱动器内部指令位置。
        ...
        # 这个 diff 是当前位置 - 指令位置，不是当前位置 - 目标位置。
    ...
    def move_relative_pp(self, slave: int, offset: int,
        ...
        # 等待当前位置等于绝对目标位置
        # 虽然写入的是 offset，但等待时用绝对位置 target_abs_pos 判断。
        start_time = time.time()
    ...
    def quick_stop(self, slave: int) -> bool:
        ...
        """
        # 当前实现写的是 CMD_SHUTDOWN，用于让运动结束后回到更安全的状态。
        if not self._write_controlword(slave, self.CMD_SHUTDOWN):
            return False
        # 给驱动器一点处理控制字的时间。
        time.sleep(0.05)
