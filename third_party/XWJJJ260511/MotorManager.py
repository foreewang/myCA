...
# logging 用于输出模式切换、使能、运动等过程日志。
import logging
# Optional 用于标注读取失败时返回 None。
from typing import Optional
# time 只在文件底部示例里用于 sleep。
import time

# 假设已有 modbus 模块中的 ModbusRTUClient 类
# ModbusRTUClient 负责真正读写驱动器寄存器。
from .modbus import ModbusRTUClient

# 当前模块日志对象，前缀会显示 XWJJJ260511.MotorManager。
...
class MotorManager:
    ...
    def __init__(self, client: ModbusRTUClient, slave: int):
        ...
        """
        # 保存底层 Modbus 客户端，所有寄存器读写都通过它完成。
        self.client = client
        # 保存当前电机的从站号；一个 client 可以管理多个从站。
        self.slave = slave

    # -------------------- 内部状态查询方法 --------------------
    def _read_statusword(self) -> Optional[int]:
        """读取状态字，失败返回 None"""
        # 状态字里包含是否使能、是否故障、是否到位等状态位。
    ...
    def _is_enabled(self) -> bool:
        ...
        # Operation enabled 位为 1 时，说明电机已经可以执行运动。
    ...
    def _ensure_mode_and_enable(self, target_mode: int,auto_enable: bool = True) -> bool:
        ...
        if current_mode == target_mode:
            # 模式正确，检查使能状态
            if auto_enable:
                # 需要自动使能时，进入 CiA402 Operation enabled 状态。
    ...
    def pv_start(self, target_velocity: int, profile_acc: int, profile_dec: int) -> bool:
        ...
        :return:                成功返回 True
        """
        # PV 模式下通常先切模式，不在这里自动使能，由底层启动逻辑处理。
...
# x y 电机点位参数，下面这些常量是托盘点位和显微镜物镜/焦距示例。
