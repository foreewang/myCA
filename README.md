...
vision 当前采用 strict dark-core 粗检测策略：
...
补偿阶段会跳过 `is_valid_for_compensation=false` 的候选，避免触边、低置信度或异常候选参与补偿。
...
- 默认触发策略是物镜发生切换后自动对焦；也可通过 `config/autofocus.yaml` 设置每次采集前强制对焦。
