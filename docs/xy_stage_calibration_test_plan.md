...
换孔使用 `well_step` 示教脉冲步长，不再使用 `well_diameter_mm + well_gap_mm`。内径只用于孔内扫描圆。
...
逐板低速验证 A1、A2、B1，再验证最远孔。软件按当前示教步长应计算为：

| 板型 | A1 | A2 | B1 | 最远孔（网格） |
| --- | ---: | ---: | ---: | ---: |
| 6-well | (6198780,7287429) | (3631627,7284136) | (6198780,2227451) | B3=(1064474,2220865) |
| 12-well | (5774368,8116360) | (4067673,8116360) | (5774368,4744226) | C4=(654283,1372092) |
| 24-well | (6110380,8508572) | (4843874,8508572) | (6107047,5995754) | D6=(-232149,970118) |
| 48-well | (5848352,8942329) | (4989792,8942332) | (5849552,7247494) | F8=(-155568,468175) |
...
使用当前物镜 FOV、`overlap=0.1` 对 A1 做离线规划。扫描圆半径为 `内径/2 − 半视野`，只覆盖培养面内径。新 A1 相对旧外径起点上移后，向上扫描会超出 Y 安全上限 `9110661`；规划会丢掉越界点并继续，上沿可能少盖一窄条。不要减小 `safety_margin` 或扩大硬限位。
...
若业务要求上沿也完整覆盖，需沿 Y 负方向重新布置板位并重测各板 A1 的 Y。起始点本身仍必须落在安全范围内，否则任务在规划阶段失败。
