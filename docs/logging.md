...
访问日志的 `task_id` 单独标识本次请求的目标任务：提交成功时取实际接受结果中的任务 ID，通过请求 state 传回中间件；状态、结果、取消及任务产物接口从已解析的 URL 路径参数取得任务 ID（即使任务不存在而返回 404，仍记录查询目标）。路径仍显示模板，任务标识沿用统一脱敏。无任务接口、无法解析请求体或未成功接受的提交请求使用 `task_id=-`，不从请求体猜测或记录尚未接受的任务。
